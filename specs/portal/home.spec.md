...
| 页脚统计 | GET /rest/cbc/aiplatform/analytics/summary | 返回累计 pageViews/visitors |
...
- **页面访问上报**：路由切换后异步上报 `path + visitorId` 到 `POST /rest/cbc/aiplatform/analytics/page-view`，失败静默，不影响用户交互
- **本地调试排除**：在 `localhost / 127.0.0.1 / 0.0.0.0` 环境不进行统计上报
- **页脚统计展示**：页脚显示累计「访问量(PV) / 访客数(UV)」，接口失败时静默展示占位 `--`
